...
10. [Визуальный анализ прогнозов](#10-визуальный-анализ-прогнозов)
...
3. Оценить SARIMAX с экзогенными переменными
...
| SARIMAX (48 экзогенных) | -9.6049 | 7668.14 | 6727.68 | 0.0000 | ❌ Переобучена |
| **Ridge** | **0.7935** | **1070.04** | **811.17** | — | ✅ **Лучшая модель** |
| Комбинированная | 0.7775 | 1110.74 | 836.83 | 0.0145 | ⚠️ Не улучшает Ridge |

**Примечание:** R²_test для Ridge в этом блоке (0.7935) отличается от блока 4 (0.9422), так как здесь Ridge пересчитана с 56 признаками (включая дополнительные лаги и сезонные переменные из секции 11).

---

## 10. Визуальный анализ прогнозов

### Сводка визуальных наблюдений

**График SARIMA vs SARIMAX (см. секцию 7):**
- Доверительный интервал SARIMA(3,2,3)×(0,0,1,12) покрывает только часть фактических точек, но в целом идет близко от них
- SARIMAX (с экзогенными) прогнозные точки выше SARIMA и значительно выше реальных данных — прогноз очень завышенный

**График Ridge vs Комбинированная модель (см. секцию 8):**
- Прогнозы Комбинированной модели и Ridge практически совпадают
- Отклонения между ними почти невизуализируются на графике
- Относительно реальных данных обе модели очень близки к ним
...
2. **SARIMAX переобучается** — 48 экзогенных переменных избыточны
3. **Комбинированная модель не улучшает Ridge** — SARIMA-коррекция не дает эффекта
...
| **Дальнейшие улучшения** | SARIMAX с 5-10 ключевыми переменными | Требуется сокращение экзогенных |
...
- [x] SARIMA модель построена с некоррелированными остатками
- [x] SARIMAX оценена (переобучение на 48 переменных)
- [x] Комбинированная модель проверена (нет улучшения)
- [ ] SARIMAX с сокращенным набором экзогенных переменных (5-10) — **требует решения**
- [ ] Финальный прогноз на 2026-2027 с использованием Ridge
- [ ] Оформление итогового отчета

---

*Документ обновлен: 14.08.2026*
