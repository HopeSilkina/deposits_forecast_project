...
|--------|---------|-----------|----------|
| **Полная Ridge (38 признаков)** | **0.9422** | **566.09** | **489.89** |
| Комбинированная (Ridge + SARIMA) | 0.9380 | 586.22 | 521.86 |
...
**Диагностика остатков комбинированной модели:**
- Ljung-Box min p = 0.2982 → ✅ Некоррелированы
- SARIMA-коррекция устранила автокорреляцию остатков

**Вывод:** Комбинированная модель НЕ улучшает прогноз Ridge. Ухудшение MAE на 6.5%. Однако SARIMA-коррекция устранила автокорреляцию остатков (LB p = 0.2982).
...
| **Полная Ridge (38 признаков, блок 4)** | **0.9422** | **566.09** | **489.89** | — | ✅ **Лучшая модель** |
| Комбинированная (Ridge + SARIMA) | 0.9380 | 586.22 | 521.86 | 0.2982 | ⚠️ Не улучшает Ridge, но остатки некоррелированы |

**Примечание:** Комбинированная модель использует проверенную полную Ridge (38 признаков) и SARIMA-коррекцию остатков. Она не улучшает прогноз, но устраняет автокорреляцию.
...
4. **Комбинированная модель не улучшает полную Ridge** — ухудшение MAE на 6.5%
...
| **Устранение автокорреляции** | Комбинированная (Ridge + SARIMA) | LB p = 0.2982, но прогноз чуть хуже |
| **SARIMAX с экзогенными** | ❌ Не рекомендуется | Переобучение при любом наборе переменных |

### Итоговое решение

- **БЛОК 4 (Полная Ridge)** — финальная модель для прогнозирования
